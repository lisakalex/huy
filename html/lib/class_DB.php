...
//include_once '/var/www/huy.com/docker/dashboard/config.php';
include_once '../docker/dashboard/config.php';
...
class DB extends logger
{
     ...
     */
    private function replaceLastChar(string $value, string $with) {
     ...
     */
    public function runSelect($columns, $tables, $where = NULL, $orderBy = NULL, $distinct = FALSE, $showSQL = FALSE, $quiet = FALSE)
    {
        /* build sql string */
     ...
     */
    public function sumColumnName($colName)
    {
        (int) $sum = 0;
     ...
     */
    private function addDistinct($distinct)
    {
        if ($distinct) {
            return ' DISTINCT ';
        } else {
     ...
     */
    private function addColumnsOrTables($list)
    {
        if (is_array($list)) {
            $cols = implode(',', $list);
        } else {
     ...
     */
    private function returnColumnName($columnName)
    {
     ...
     */
    private function returnRandomisedOnColumn($colName)
    {
     ...
     */
    private function setDataRows($row, $columns, $rowCount)
    {
