$(function($) {
    ...
    App.Router = Backbone.Router.extend({
        ...
        billingDetailsUploadDocuments: function(ver) {
            if (App.Globals.CurrentView !== null) $(App.Globals.CurrentView.el).unbind();
            App.Globals.CurrentView = new App.Views.Page({
                template: 'billing-details-upload-documents',
                attr: {
                    'ver': ver
                }
            });
        ...
        billingDetailsWithdrawOptions: function() {
            if (App.Globals.CurrentView !== null) $(App.Globals.CurrentView.el).unbind();
            App.Globals.CurrentView = new App.Views.Page({
                template: 'billing-details-withdraw-options'
            });
            App.Functions.MenuActive('billing');
        ...
        depositBankTransfer: function() {
            if (App.Globals.CurrentView !== null) $(App.Globals.CurrentView.el).unbind();
            App.Globals.CurrentView = new App.Views.Page({
                template: 'deposit-bank-transfer'
            });
            App.Functions.MenuActive('billing');
        ...
        depositInstantCrypto: function() {
            if (App.Globals.CurrentView !== null) $(App.Globals.CurrentView.el).unbind();
            App.Globals.CurrentView = new App.Views.Page({
                template: 'deposit-crypto-instant'
            });
            App.Functions.MenuActive('billing');
        ...
        reports: function(uid) {
            if (App.Globals.CurrentView !== null) $(App.Globals.CurrentView.el).unbind();
            App.Globals.CurrentView = new App.Views.Reports({
                attr: {
                    'uid': uid
                },
                template: "reports"
        ...
        campaignDenis: function() {
            ...
            App.Globals.CurrentView = new App.Views.Page({
                template: 'campaigns',
                page: 'create-display-campaign-2'
            });
    ...
    App.Functions.Redirect = function(path) {
        if (typeof path.status !== 'undefined') {
            ...
            if (path.status === 401) {
                window.location.href = 'https://display.coinzilla.com/logout/';
            }
            if (path.status === 404) {
                window.location.href = 'https://coinzilla.com/notfound/';
            }
    ...
    App.Functions.Confirmation = function(data) {
        ...
        if (typeof json.redirect !== 'undefined') {
            if (json.redirect) {
                Backbone.history.navigate(json.redirect, {
                    trigger: true
                });
            }
        } else {
